...
func demandPTCallback(clt *sxutil.SXServiceClient, sp *api.Demand) {
//	sp.GetArg_PTService()
}


func subscribePTDemand(client *sxutil.SXServiceClient) {
...
func pointDistance(p1 *Point, lat2 float32, lon2 float32) float32{
...
//		fmt.Printf("C1:%d, C2:%d \n",c1, c2)
		for i := c1; i <c2; i++ {
			pt := shp.Points[i]
			pt2 := shp.Points[i+1]
			totalDist += pointDistance(&Point{Latitude:float64(pt.Lat),Longitude:float64(pt.Lon)},
							pt2.Lat, pt2.Lon)
		}
		...
		for i := c1; i <c2; i++ {
			pt := shp.Points[i]
			pt2 := shp.Points[i+1]
			diff := pointDistance(&Point{Latitude:float64(pt.Lat),Longitude:float64(pt.Lon)},
				pt2.Lat, pt2.Lon)
...
func calcAngle(lat1 float32, lon1 float32, lat2 float32, lon2 float32) float32{
	...
	lat0 :=(deg2frad(lat1)+deg2frad(lat2))/2.0
	dlat := deg2frad(lat1)-deg2frad(lat2)
	dlon := deg2frad(lon1)-deg2frad(lon2)
...
func supplyPTransitFeed(clt *sxutil.SXServiceClient, feed *gtfsparser.Feed){
...
//					fmt.Printf("SameAngle : %.2f  ==",angle)
				...
				ptsdata , _ := proto.Marshal(pts)
				cont := api.Content{Entity: ptsdata}
...
// here for usual provider
	srvAddr, err := sxutil.RegisterNode(*nodesrv, "PTransit:"+*feedName,channelTypes, nil )


	go sxutil.HandleSigInt()
	sxutil.RegisterDeferFunction(sxutil.UnRegisterNode)

	client := sxutil.GrpcConnectServer(srvAddr)
	
	sclient := sxutil.NewSXServiceClient(client, pbase.PT_SERVICE,"{Clt:GTFS}")
